...
int main(int argc, char *argv[]) {
  int flagError = 0;
  Options options = catReadOptions(argc, argv, &flagError);
  if (!flagError) {
    ...
    flagError = Cat(argc, argv, options, table);
    if (flagError) {
...
Options catReadOptions(int argc, char *argv[], int *flagOpt) {
...
int Cat(int argc, char *argv[], Options options,
        const char *table[static 256]) {
  int flagNoFile = 1;
  for (int i = optind; i < argc; i++) {
    FILE *file = fopen(argv[i], "r");
    if (file != NULL) {
      CatFile(file, options, table);
      fclose(file);
    } else {
      flagNoFile = 1;
    }
...
void errorPrinting(int *flagError) {
  if (*flagError) {
    fprintf(stderr, "illegal option, use -b -e -v -n -s -t \n");
  } else {
